...
gulp.task('lessc', function () {
	return gulp.src('source/less/EXOTouch.less')
		.pipe(less({compress: true}))
		.pipe(gulp.dest('octoprint_EXOTouch/static/css'));
});
...
gulp.task("less:concat", function() {
	return gulp.src('source/less/EXOTouch.less')
		...
		.pipe(strip())
		.pipe(rename("EXOTouch.bundled.less"))
		.pipe(trimlines())
		...
		.pipe(gulp.dest('octoprint_EXOTouch/static/less/'));
});

gulp.task('clean:hash', function () {
	return del([
		'octoprint_EXOTouch/static/css/hash.EXOTouch',
	]);
});
...
gulp.task('js:concat:libs', function () {
	return gulp.src([
		...
		.pipe(concat('EXOTouch.libraries.js'))
		.pipe(gulp.dest('octoprint_EXOTouch/static/js/'));
});
...
gulp.task('js:concat:app', function () {
	return gulp.src([
		...
		.pipe(concat('EXOTouch.bundled.js'))
		//.pipe(uglify())
		.pipe(gulp.dest('octoprint_EXOTouch/static/js/'));
});
...
gulp.task('js:concat:bootstrap', function () {
	return gulp.src([
		...
		.pipe(rename("EXOTouch.bootstrap.js"))
		.pipe(uglify())
		.pipe(gulp.dest('octoprint_EXOTouch/static/js/'));
});
