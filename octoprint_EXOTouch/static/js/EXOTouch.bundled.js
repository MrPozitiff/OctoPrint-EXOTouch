...
EXOTouch.prototype = {
	...
	settings: {
		...
		isChromiumArm: (window.navigator.userAgent.indexOf("X11") !== -1 && window.navigator.userAgent.indexOf("Chromium") !== -1 && window.navigator.userAgent.indexOf("armv7l") !== -1 || window.navigator.userAgent.indexOf("EXOTouch") !== -1),
		...
		EXOTouchModal: $('#EXOTouch_settings_dialog'),
...
EXOTouch.prototype.animate.hide = function(what) {
	var self = this;
...
EXOTouch.prototype.components.dropdown = {
	...
	toggle: function() {
		var self = this;
		var namespace = ".EXOTouch.dropdown";
...
EXOTouch.prototype.components.fullscreen = {
	init: function() {
		var self = this;
...
EXOTouch.prototype.components.keyboard = {
...
EXOTouch.prototype.components.modal = {
				...
				.on("click", function(e) {
					...
					$(document).on("click", function(event) {
						...
						if(
							...
							$(document).off(event).trigger("dropdown-closed.EXOTouch"); // Trigger event for enabling scrolling
					...
					$(document).trigger("dropdown-open.EXOTouch", elm[0]);
				});

			// reset the active text in dropdown on open
			$(appendTo)
				.closest(".modal")
				.on("modal.EXOTouch", function() {
					var href = $(cloneId)
						.find(".active")
...
EXOTouch.prototype.components.slider = {
...
EXOTouch.prototype.components.touchList = {
	init: function() {
...
EXOTouch.prototype.components.touchscreen = {
	...
	isLoading: function (viewModels) {
		...
		if(this.settings.isTouchscreen()) {
			...
			if (viewModels.gcodeViewModel) {
				console.info("EXOTouch: Disabling GCodeViewer in touchscreen mode...");
...
EXOTouch.prototype.core.boot = function() {

	// This should always start EXOTouch
	if(
...
EXOTouch.prototype.core.less = {

	options: {
		template: {
			importUrl:	"./plugin/EXOTouch/static/less/EXOTouch.bundled.less?t=" + new Date().getTime(),
	...
	error: function(error) {
		...
		if(content && content.trim() && error.status !== 401) {
			new PNotify({
				title: 'EXOTouch: Whoops, something went wrong...',
				text: content,
				icon: 'glyphicon glyphicon-question-sign',
				type: 'error',
				hide: false
...
EXOTouch.prototype.DOM.cookies = {

	get: function(key, isPlain) {
		var name = (isPlain) ? key + "=" : "EXOTouch." + key + "=";
...
EXOTouch.prototype.DOM.localstorage = {
	store: JSON.parse(localStorage["EXOTouch"] || "{}"),
	...
	set: function (key, value) {
		...
		localStorage["EXOTouch"] = JSON.stringify(this.store);
		return this.store[key];
	},
...
EXOTouch.prototype.DOM.init = function() {
	...
	$('#tabs [data-toggle=tab]').on("click", function() {
		$("#all_EXOTouch_settings").removeClass("item_active");
	});

}

EXOTouch.prototype.knockout.bindings = function() {
	var self = this;
	...
	this.bindings = {
		...
		show: function() {
			self.settings.EXOTouchModal.modal("show");
		}
...
EXOTouch.prototype.knockout.isLoading = function (viewModels) {
	var self = this;
	...
	if(self.isActive()) {
		...
		viewModels.connectionViewModel.isOperational.subscribe(function(newOperationalState) {
			var printLink = $("#all_EXOTouch_settings");
...
EXOTouch.prototype.knockout.viewModel = function() {
	var self = this;
	...
	self.onBeforeBinding = function() {
		ko.mapping.fromJS(self.core.bridge.allViewModels.settingsViewModel.settings.plugins.EXOTouch, {}, self.settings);
	}
...
EXOTouch.prototype.scroll.blockEvents = {
...
EXOTouch.prototype.scroll.body = {
...
EXOTouch.prototype.scroll.modal = {
	stack: [],
	dropdown: null,
	...
	init: function() {
		...
		$document.on("modal.EXOTouch", function(e, elm) {
			var $modalElm = $(elm),
			...
			$document.on("click.scrollHeightEXOTouch", '[data-toggle="tab"], .pagination ul li a', function(e) {
				curModal._end(e);
			...
			$modalElm.one("destroy", function() {
				$document.off("click.scrollHeightEXOTouch");
		...
		$document.on("dropdown-open.EXOTouch", function(e, elm) {
			var $elm = $(elm);
			...
			$document.on("dropdown-closed.EXOTouch", function() {
				// Enable active modal
...
EXOTouch.prototype.scroll.overlay = {
...
EXOTouch.prototype.scroll.overwrite = function(terminalViewModel) {
	var self = this;
...
EXOTouch.prototype.scroll.terminal = {
...
EXOTouch.prototype.scroll.beforeLoad = function() {
...
EXOTouch.prototype.scroll.init = function() {
	var self = this;
...
EXOTouch.prototype.plugins.navbarTemp = function() {
...
EXOTouch.prototype.plugins.screenSquish = function(pluginManagerViewModel) {
	var shown = false;
	...
	pluginManagerViewModel.plugins.items.subscribe(function() {
		...
		if(!shown && ScreenSquish && ScreenSquish.enabled) {
			shown = true;
			new PNotify({
				title: 'EXOTouch: ScreenSquish is running',
				text: 'Running ScreenSquish and EXOTouch will give issues since both plugins try the same, we recommend turning off ScreenSquish.',
				icon: 'glyphicon glyphicon-question-sign',
				type: 'error',
				hide: false,
				confirm: {
...
EXOTouch.prototype.DOM.create.dropdown = {
	...
	init: function() {
		...
		this.menuItem.menu = $('' +
			'<li id="all_EXOTouch_settings" class="dropdown">' +
...
EXOTouch.prototype.DOM.create.printer = {
...
EXOTouch.prototype.DOM.create.tabbar = {
...
EXOTouch.prototype.DOM.create.webcam = {
...
EXOTouch.prototype.DOM.move.afterTabAndNav = function() {
...
EXOTouch.prototype.DOM.move.controls = {
...
EXOTouch.prototype.DOM.move.navbar = {
	mainItems: ['#all_EXOTouch_settings', '#navbar_plugin_navbartemp', '#navbar_login', /*'#navbar_systemmenu',*/ '.hidden_touch'],
	init: function() {
		...
		// Move EXOTouch to main dropdown
		$("#navbar_plugin_EXOTouch").insertAfter("#navbar_settings");

		// Create and Move login form to main dropdown
		$('<li><ul id="youcanhazlogin"></ul></li>').insertAfter("#navbar_plugin_EXOTouch");
...
EXOTouch.prototype.DOM.move.overlays = {
...
EXOTouch.prototype.DOM.move.tabbar = {
	...
	init: function() {
		...
		$items.each(function(ind, elm) {
			...
			$elm.clone().attr("id", $elm.attr("id")+"2").appendTo("#all_EXOTouch_settings .dropdown-menu").find('a').off("click").on("click", function(e) {
				$elm.find('a').click();
				$("#all_EXOTouch_settings").addClass("item_active");
				e.preventDefault();
				return false;
			});
...
EXOTouch.prototype.DOM.move.terminal = {
...
EXOTouch.prototype.DOM.overwrite.modal = function() {
	...
	$.fn.modal = function(options, args) {
		...
		if (options !== "hide") {
			$(this).trigger("modal.EXOTouch", this);
		}
		return tmp;
	};
...
EXOTouch.prototype.DOM.overwrite.tabbar = function() {
